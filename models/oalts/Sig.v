...
Module Sig <: BicartesianCategory.
  ...
  (** It is worth hashing out what events are available in [«A -o B»].
    ...
    - [x] is an environment event and comes from [A^+ + B^-]
    - [y] is a system event and comes from [A^- + B^+]
    
    This means that events are one of:

    Asynchronous events:
    - [⟨a^+ | ⟩]: A positive event in [A] performed by env
    - [⟨b^- | ⟩]: A negative event in [B] performed by env
    - [⟨ | a^-⟩]: A negative event in [A] performed by sys
    - [⟨ | b^+⟩]: A positive event in [B] performed by sys

    Synchronous events:
    - [⟨a^+ | a^-⟩]: A positive event in [A] performed by env synchronized
      with a negative event in [A] performed by the system
    - [⟨b^- | b^+⟩]: A negative event in [B] performed by env synchronized 
      with a positive event in [B] performed by the system
    - [⟨b^- | a^-⟩]: A negative event in [B] performed by env synchronized 
      with a negative event [A] event performed by sys
    - [⟨a^+ | b^+⟩]: A positive event in [A] performed by env synchronized
      with a positive event [B] performed by sys    
  *)
